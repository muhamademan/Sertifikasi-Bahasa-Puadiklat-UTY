                ...
                <table class="table table-hover" id="example">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="text-dark"><?= $p['nama_lengkap']; ?> <a href="#" class="text-info"
                                        data-toggle="modal" data-target="#detailPeserta<?= $p['id'] ?>"><i
                            ...
                            <td class="text-center">
                                <?php if ($p['presensi'] == 0) : ?>
                                <!-- Absensi -->
                                <a href="<?= base_url('ptr/hadir/') . $p['id'] . '/' . $kelas_id; ?>"
                                    class="btn btn-success">Hadir</a>
                                <?php else : ?>
                                ...
                                <form action="<?= base_url('ptr/inputnilai') ?>" method="post">
                                    <div class="input-group">
                                        ...
                                        <input type="number" name="nilai" class="form-control" id="search-input" min="0"
                                            max="1000" placeholder="Nilai" required>
                                        <?php else : ?>
                                        <input type="number" name="nilai" class="form-control" id="search-input" min="0"
                                            max="100" placeholder="Nilai" required>
                                        <?php endif; ?>
                                        <div class="input-group-append">
                                            <button class="btn btn-success" type="submit"
                                                id="search-button">Simpan</button>
                                        </div>
                                        <div class="input-group-append">
                                            <a href="<?= base_url('ptr/belumhadir/') . $p['id'] . '/' . $kelas_id; ?>"
                                                class="btn btn-danger" type="submit" id="search-button">X</a>
                                        </div>
                                    </div>
                                </form>
                                <?php else : ?>
                                <span class="text-dark"><?= $p['nilai'] ?></span><a
                                    href="<?= base_url('ptr/hapusnilai/') . $p['id'] . '/' . $kelas_id; ?>"
...
<div class="modal fade" id="detailPeserta<?= $p['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
    aria-hidden="true">
